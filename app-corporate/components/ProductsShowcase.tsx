'use client';

import Link from 'next/link';
import { ArrowRight, Sparkles, Heart, Phone, Target, CheckCircle2 } from 'lucide-react';
import { useLanguage } from '../context/LanguageContext';
import { useReveal } from '../hooks/useReveal';

export default function ProductsShowcase() {
  const { t, lang } = useLanguage();
  const basePath = `/${lang}`;
  const { ref, isVisible } = useReveal();

  const products = [
    {
      nameKey: 'products.p1Name',
      descKey: 'products.p1Desc',
      tagKey: 'products.p1Tag',
      href: `${basePath}/content-factory`,
      status: 'live',
      icon: Sparkles,
      gradient: 'from-violet-500 to-purple-500',
      glow: 'rgba(139, 92, 246, 0.35)',
      accent: '#8b5cf6',
    },
    {
      nameKey: 'products.p2Name',
      descKey: 'products.p2Desc',
      tagKey: 'products.p2Tag',
      href: `${basePath}/cases/sweezy`,
      status: 'beta',
      icon: Heart,
      gradient: 'from-pink-500 to-rose-500',
      glow: 'rgba(236, 72, 153, 0.35)',
      accent: '#ec4899',
    },
    {
      nameKey: 'products.p3Name',
      descKey: 'products.p3Desc',
      tagKey: 'products.p3Tag',
      href: `${basePath}/products`,
      status: 'live',
      icon: Phone,
      gradient: 'from-emerald-500 to-teal-500',
      glow: 'rgba(16, 185, 129, 0.35)',
      accent: '#10b981',
    },
    {
      nameKey: 'products.p4Name',
      descKey: 'products.p4Desc',
      tagKey: 'products.p4Tag',
      href: `${basePath}/products`,
      status: 'coming_soon',
      icon: Target,
      gradient: 'from-amber-500 to-orange-500',
      glow: 'rgba(249, 115, 22, 0.35)',
      accent: '#f97316',
    },
  ];

  const statusLabel = (s: string) =>
    s === 'live' ? t('products.live') : s === 'beta' ? t('products.beta') : t('products.comingSoon');

  const statusClass = (s: string) =>
    s === 'live'
      ? 'bg-emerald-500/15 text-emerald-300 border-emerald-500/30'
      : s === 'beta'
      ? 'bg-amber-500/15 text-amber-300 border-amber-500/30'
      : 'bg-white/[0.06] text-gray-300 border-white/15';

  return (
    <section id="products" className="relative py-24 px-6 overflow-hidden content-visibility-auto">
      <div className="absolute inset-0 overflow-hidden">
        <div
          className="absolute inset-0"
          style={{
            background: `
              radial-gradient(ellipse 60% 40% at 30% 20%, rgba(139, 92, 246, 0.06) 0%, transparent 50%),
              radial-gradient(ellipse 50% 40% at 70% 80%, rgba(236, 72, 153, 0.05) 0%, transparent 50%)
            `,
          }}
        />
      </div>

      <div ref={ref} className="relative max-w-7xl mx-auto">
        <div className={`text-center mb-14 reveal ${isVisible ? 'visible' : ''}`}>
          <div className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full mb-6 border border-violet-500/25 bg-violet-500/[0.08]">
            <span className="relative flex h-2 w-2">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-violet-400 opacity-75" />
              <span className="relative inline-flex rounded-full h-2 w-2 bg-violet-500" />
            </span>
            <span className="text-sm font-semibold text-violet-300 uppercase tracking-wider">
              {t('homeProducts.badge')}
            </span>
          </div>

          <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold font-heading mb-5 leading-[1.1]">
            <span className="text-white">{t('homeProducts.title1')}</span>
            <br />
            <span
              style={{
                background: 'linear-gradient(135deg, #8b5cf6 0%, #ec4899 50%, #f97316 100%)',
                WebkitBackgroundClip: 'text',
                WebkitTextFillColor: 'transparent',
              }}
            >
              {t('homeProducts.title2')}
            </span>
          </h2>

          <p className="text-lg md:text-xl text-gray-400 max-w-2xl mx-auto">
            {t('homeProducts.subtitle')}
          </p>
        </div>

        <div className="grid md:grid-cols-2 gap-5">
          {products.map((p, idx) => {
            const Icon = p.icon;
            return (
              <div
                key={idx}
                className={`reveal reveal-delay-${Math.min(idx + 1, 4)} ${isVisible ? 'visible' : ''} group relative`}
              >
                <Link href={p.href} className="block h-full">
                  <div className="relative h-full rounded-[1.5rem] border border-white/10 bg-gradient-to-br from-white/[0.05] to-white/[0.01] overflow-hidden transition-all duration-300 hover:border-white/20 hover:-translate-y-1">
                    <div
                      className={`absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r ${p.gradient} opacity-60 group-hover:opacity-100 transition-opacity duration-300`}
                    />
                    <div
                      className="absolute top-0 right-0 w-40 h-40 opacity-15 group-hover:opacity-25 transition-opacity duration-300"
                      style={{
                        background: `radial-gradient(circle at top right, ${p.accent} 0%, transparent 70%)`,
                      }}
                    />

                    <div className="relative z-10 p-6 lg:p-7">
                      <div className="flex items-start justify-between mb-5">
                        <div
                          className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${p.gradient} flex items-center justify-center shadow-lg transition-transform duration-300 group-hover:scale-110`}
                          style={{ boxShadow: `0 8px 24px ${p.glow}` }}
                        >
                          <Icon className="w-7 h-7 text-white" />
                        </div>
                        <span
                          className={`text-[11px] font-bold uppercase tracking-wider px-3 py-1.5 rounded-full border ${statusClass(p.status)}`}
                        >
                          {statusLabel(p.status)}
                        </span>
                      </div>

                      <div className="mb-1.5 text-xs font-semibold uppercase tracking-wider text-gray-500">
                        {t(p.tagKey)}
                      </div>
                      <h3 className="text-xl lg:text-2xl font-bold text-white mb-3 leading-tight">
                        {t(p.nameKey)}
                      </h3>
                      <p className="text-sm lg:text-base text-gray-400 leading-relaxed mb-5 line-clamp-2">
                        {t(p.descKey)}
                      </p>

                      <div className="flex items-center justify-between pt-4 border-t border-white/10">
                        <div className="flex items-center gap-2 text-sm text-gray-500">
                          <CheckCircle2 className="w-4 h-4" style={{ color: p.accent }} />
                          <span>{statusLabel(p.status)}</span>
                        </div>
                        <div
                          className="flex items-center gap-1.5 text-sm font-semibold transition-all duration-300 group-hover:gap-2.5"
                          style={{ color: p.accent }}
                        >
                          <span>{t('products.viewProduct')}</span>
                          <ArrowRight className="w-4 h-4 transition-transform duration-300 group-hover:translate-x-1" />
                        </div>
                      </div>
                    </div>
                  </div>
                </Link>
              </div>
            );
          })}
        </div>

        <div className={`text-center mt-10 reveal ${isVisible ? 'visible' : ''}`} style={{ transitionDelay: '400ms' }}>
          <Link href={`${basePath}/products`} className="btn-primary px-8 py-4 text-base">
            <span>{t('homeProducts.viewAll')}</span>
            <ArrowRight className="w-5 h-5" />
          </Link>
        </div>
      </div>
    </section>
  );
}
