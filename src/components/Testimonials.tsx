          ...
          {testimonials.map((item, i) => (
            <motion.div
              ...
              className={`glass-card rounded-2xl p-6 sm:p-8 transition-all duration-300 ${
                item.featured ? "border-[#0ea5e9]/20 md:scale-[1.02]" : ""
              }`}
            >
              ...
              <div className="glass-card rounded-xl p-4 mb-6">
                <div className="flex items-baseline gap-2">
                  <span className="text-2xl font-bold gradient-text">{item.metric}</span>
                  <span className="text-xs text-[var(--color-text-muted)]">{item.metricLabel}</span>
                </div>
              </div>
